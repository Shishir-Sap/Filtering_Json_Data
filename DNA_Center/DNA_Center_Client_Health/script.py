...
def get_token(host):
    ...
    url_token_req = f"https://{host}/dna/system/api/v1/auth/token"
    auth_resp = requests.post(
        url_token_req, auth=auth, headers=headers, verify=False)
...
url_req = f"https://{host}/dna/intent/api/v1/client-health"
get_resp = requests.get(url_req, headers=headers, verify=False)
